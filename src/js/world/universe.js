...
export class Universe {
  ...
  createPlayerShip() {
    ...
    this.ships.push(
      (this.playerShip = new PlayerShip(this.center.x, this.center.y))
    );
  }
  ...
  cycle(e) {
    ...
    this.forEach(
      [this.bodies, this.ships, this.items, this.projectiles, [V]],
      element => element.cycle(e)
    );
  }
  ...
  render() {
    ...
    wrap(() => {
      ...
      for (let i = 0; i < 400; i++) {
        wrap(() => {
          ...
          translate(
            moduloWithNegative(
              rng.between(-1, 1) * CANVAS_WIDTH -
                U.playerShip.x * distanceFactor,
              CANVAS_WIDTH
            ),
            moduloWithNegative(
              rng.between(-1, 1) * CANVAS_HEIGHT -
                U.playerShip.y * distanceFactor,
              CANVAS_HEIGHT
            )
          );
    ...
    wrap(() => {
      ...
      this.forEach(
        [this.projectiles, this.particles, this.ships, this.bodies, this.items],
        element =>
          wrap(() => {
            element.render();
          })
  ...
  generateUniverse() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let i = 0; i < maxSystems; i++) {
        ...
        const planets = rng.between(
          UNIVERSE_GENERATE_SYSTEM_MIN_PLANETS,
          UNIVERSE_GENERATE_SYSTEM_MAX_PLANETS
        );
        let orbitRadius = rng.between(
          UNIVERSE_GENERATE_ORBIT_MIN_MARGIN,
          UNIVERSE_GENERATE_ORBIT_MAX_MARGIN
        );
