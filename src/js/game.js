...
export class Game {
  ...
  proceedToMissionStep(missionStep) {
    ...
    missionStep.proceedListener = missionStep =>
      G.proceedToMissionStep(missionStep);
    missionStep.attach();
  }
  ...
  cycle(e) {
    ...
    wrap(() => {
      ...
      G.renderGauge(
        ...
        50,
        U.playerShip.health,
        U.playerShip.health < 0.25 || G.clock - U.playerShip.lastDamage < 0.2
          ? "#f00"
          : G.healthGaugeColor,
        () => {
      ...
      G.renderGauge(
        100,
        80,
        U.playerShip.civilization.resources / PLANET_MAX_RESOURCES,
        "#fff",
        () => {
      ...
      G.renderGauge(
        ...
        110,
        U.playerShip.heat,
        U.playerShip.coolingDown ? "#f00" : "#fff",
        () => {
          fr(-5, -5, 3, 10);
          fr(-1, -5, 3, 10);
          fr(3, -5, 3, 10);
      ...
      targets.forEach(target => {
        ...
        wrap(() => {
          const distanceOnCircle =
            limit(
              0,
              (dist(target, U.playerShip) - target.reachRadius) / 4000,
              1
            ) *
              200 +
            50;
          ...
          translate(
            CANVAS_WIDTH / 2 + cos(angle) * distanceOnCircle,
            CANVAS_HEIGHT / 2 + sin(angle) * distanceOnCircle
          );
          rotate(angle);
        ...
        wrap(() => {
          ...
          if (actualText.length >= promptText.length) {
            ...
            G.promptOptions.forEach((option, i) => {
              ...
              const promptText =
                "[" + option.label[0] + "]" + option.label.slice(1);
              const x = (i + 1) * (CANVAS_WIDTH / (G.promptOptions.length + 1));

              fr(
                x - PROMPT_OPTION_BOX_WIDTH / 2,
                100 - PROMPT_OPTION_BOX_HEIGHT / 2,
                PROMPT_OPTION_BOX_WIDTH,
                PROMPT_OPTION_BOX_HEIGHT
              );
      ...
      if (G.message && G.messageProgress) {
        wrap(() => {
          ...
          translate(
            (CANVAS_WIDTH - messageWidth) / 2,
            (CANVAS_HEIGHT - 100) / 2 - 200
          );
      ...
      wrap(() => {
        ...
        wrap(() => {
          translate(
            (CANVAS_WIDTH - G.titleStickString.width * G.titleCharWidth) / 2,
            everyonesY
          );
          renderStickString(
            G.titleStickString,
        ...
        wrap(() => {
          ...
          translate(
            (CANVAS_WIDTH - G.subtitleStickString.width * G.subtitleCharWidth) /
              2,
            everyonesY + (G.titleCharHeight * 7) / 5
          );
          renderStickString(
            G.subtitleStickString,
            G.subtitleCharWidth,
            G.subtitleCharHeight,
            G.clock - 0.5,
            0.1 *
              (G.titleStickString.segments.length /
                G.subtitleStickString.segments.length),
            1
          );
        });
        ...
        wrap(() => {
          ...
          translate(
            (CANVAS_WIDTH -
              G.instructionsStickString.width * instructionCharWidth) /
              2,
            CANVAS_HEIGHT - instructionCharHeight - 100
          );
          renderStickString(
            G.instructionsStickString,
            instructionCharWidth,
            instructionCharHeight,
            G.clock - 5,
            0.01,
            0.2
          );
  ...
  showPrompt(promptText, options) {
    G.promptText =
      promptText && promptText.call ? promptText : () => promptText;
  ...
  selectPromptOption(characterOrIndex) {
    ...
    if (
      actualText.length < (G.promptText() || "").length ||
      G.selectedPromptOption
    ) {
      return;
    }
    ...
    (G.promptOptions || []).forEach((option, i) => {
      if (
        i == characterOrIndex ||
        option.label[0].toLowerCase() === characterOrIndex
      ) {
  ...
  showMessage(message) {
    ...
    interp(
      G,
      "messageProgress",
      G.message.segments.length,
      0,
      G.message.segments.length * 0.1,
      3
    );
    interp(
      G,
  ...
  promptRandomMission() {
    ...
    const planet = U.bodies
      ...
      .reduce(
        (closest, body) =>
          !closest || dist(U.playerShip, body) < dist(U.playerShip, closest)
            ? body
            : closest,
        null
      );
    ...
    if (planet && !G.missionStep) {
      ...
      const otherPlanets = () =>
        U.bodies.filter(body => body.orbitsAround).filter(close);
      const otherPlanetAndStars = () =>
        otherPlanets().concat(U.stars.filter(close));
      ...
      for (
        let i = 0,
          d = max(planet.radius, dist(U.playerShip, planet) - V.visibleWidth);
        d < dist(U.playerShip, planet);
        i++, d += 50
      ) {
  ...
  missionDone(success) {
    ...
    missionStep.civilization.updateRelationship(
      success
        ? RELATIONSHIP_UPDATE_MISSION_SUCCESS
        : RELATIONSHIP_UPDATE_MISSION_FAILED
    );
  ...
  gameOver() {
    const civilizations = U.bodies
      .filter(
        body =>
          body.civilization &&
          body.civilization.relationshipType() !=
            body.civilization.initialRelationship
      )
      .map(body => body.civilization);

    const enemiesMade = civilizations.filter(
      civilization => civilization.relationshipType() == RELATIONSHIP_ENEMY
    ).length;
    const alliesMade = civilizations.filter(
      civilization => civilization.relationshipType() == RELATIONSHIP_ALLY
    ).length;
    ...
    interp(G, "titleYOffset", -CANVAS_HEIGHT, 0, 0.3, 0, 0, () =>
      G.setupNewGame()
    );
  ...
  currentPromptText() {
    ...
    return (
      promptText.slice(0, length) +
      (length < promptText.length || G.clock % 1 > 0.5 ? "_" : "")
    );
  }
}
